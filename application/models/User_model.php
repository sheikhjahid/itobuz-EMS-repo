...
class user_model extends CI_Model 
{
	...
	public function checkUser($username,$password)
	{
		...
		public function insertLeaveData($table='')
		{
			$this->db->join('users');
		}//end of function
		...
		{
			$this->db->select('users.fullname as user_name,users.id as user_id,leave-table.*,types-leave.type  as leave_name');
			$this->db->join('users','users.id=leave-table.user_id');
			$this->db->join('types-leave','types-leave.id=leave-table.leave_id');
			$query=$this->db->get('leave-table');
			return  $query->result();
		}//end of function
		public function showleaveByStatus($status=1)
		{
			$this->db->select('users.fullname as user_name,users.id as user_id,leave-table.*,types-leave.type as leave_name');
			$this->db->join('users','users.id=leave-table.user_id');
			$this->db->join('types-leave','types-leave.id=leave-table.leave_id');
			$this->db->where('leave-table.status',$status);
			$query=$this->db->get('leave-table');
			return  $query->result();
		}//end of function

		public function expiredLeaveCheck()
		{
			$today=date('Y-m-d');
		    ...
		    {
			$query_expired=$this->db->get('leave-table');
		    }//end of if
			return $query_expired->result();
		}//end of function
